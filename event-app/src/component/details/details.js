...
function Details(props) {
  const [searchstar, setSearchstar] = useState({ rating: null });
  ...
  const getRating = () => {
    axios.post("/api/rating/getRating", variabl).then((res) => {
      if (res.data.success) {
  ...
  const getRatingNumber = () => {
    axios.post("/api/rating/RatingNumber", variabl).then((res) => {
      if (res.data.success) {
        setSearchstar({
          rating:
            res.data.rating.reduce((accum, item) => accum + item.numbre, 0) /
            res.data.RatingNumber,
        });
        console.log(
          res.data.rating.reduce((accum, item) => accum + item.numbre, 0) /
            res.data.RatingNumber
  ...
  const onStarClicktwo = (nextValue) => {
    if (!Rated) {
      let variable = {
        userId: userId.id,
      ...
      axios.post("/api/rating/addRating", variable).then((res) => {
        if (res.data.success) {
          alert("Votre note est envoyé");
          getRatingNumber();
        }
      });
    } else {
      alert("vous avez déjà fait");
    }
  };
  const onDislike = () => {
    if (DislikeAction !== null) {
              ...
              >
               {event.imagemap && (
                  <img
                    ...
                    src={`http://localhost:5000/${event.imagemap}`}
                    alt="map"
                  />
                )}
              </div>
                ...
                {currentAdh && currentUser && (
                  ...
                  >
                    <CardContent>
                      ...
                      <div style={{}}>
                        <h6 style={{ marginBottom: 15, marginTop: 15 }}>
                          Email:&nbsp;&nbsp;&nbsp;&nbsp; {currentUser.email}
                        </h6>
                        <h6 style={{ marginBottom: 15 }}>
                          Tel:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{currentAdh.Phone}
                        </h6>
                        <h6 style={{ marginBottom: 15 }}>
                          Gouvernorat:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{" "}
            ...
            <GridItem style={{ marginBottom: 100 }} xs={12}>
              <div>
                ...
                >
                  ...
                  <Button
                    onClick={onSubmit}
                    style={{ width: "10%", height: "52px", marginLeft: 15 }}
                  >
                    Submit
                  </Button>
                </form>
